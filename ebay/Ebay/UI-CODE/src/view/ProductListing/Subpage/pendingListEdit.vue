...
import { setPendingEditSku, getPendingEditSku } from '@/utils/auth'
...
export default {
  ...
  // },
  activated() {
    let id = this.$route.query.sku  
    if (JSON.parse(getPendingEditSku())) {
      if (id != JSON.parse(getPendingEditSku())) { //如果是点击标签页面进来得，带得是不同id，那么应该重新发起请求，并清空push进去的数据，避免重复           
                let query = { listing_id: id }
                setTimeout(() => {
                  this.getProductInfo(query)
                }, 500)
  ...
  beforeRouteLeave(to, from, next) { // 路由离开的时候，将当前路由参数存入缓存，和下一次的进行比较
      if (this.$route.query.sku) setPendingEditSku(JSON.stringify(this.$route.query.sku))     
      next() 
  },
  mounted() {
  ...
  methods: {
      ...
      this.$refs[form].validate((valid) => {
          if (valid) {
            ...
            if (this.form.product2) {
              ...
              this.$store.dispatch('listing/savePendingEdit', data)
          } else {
             this.$message({
              type: 'warning',
              message: '请填写必填的字段'
    ...
    // 取消操作
    cancel() {
      this.$router.push('/productListing/pendingList')
    },
    // 删除产品属性
    ...
    getProductInfo(query) {
        ...
        getListingPublishrecordDetail(query).then(res => {
          ...
          let list = []
            ...
            this.form.gj_quantity = parseInt(data.gj_quantity) ? parseInt(data.gj_quantity) : ''
            if (this.form.salesMethod === 0) {
            ...
            this.form.richTextContent = `<div>${data.description.replace(/↵↵/, "<p></p>")}</div>`
