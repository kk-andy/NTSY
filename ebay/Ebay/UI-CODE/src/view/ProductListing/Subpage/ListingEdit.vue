    ...
    <el-form :inline="true" :model="form" ref="form" :rules="rules" class="demo-form-inline" label-width="135px">
      <div>
        ...
        <div>
          <el-form-item label="SPU：" label-width="135px">
            <el-input v-model="form.spu"  placeholder="" style="width: 217px" disabled></el-input>
          </el-form-item>
          <el-form-item>
          ...
          <el-form-item label="店铺：" style="width:100%" prop="shop" label-width="135px">
            <el-select v-model="form.shop"  placeholder="请选择店铺" disabled>
              <el-option
          ...
          <el-form-item label="站点：" style="width:100%" prop="site" label-width="135px">
            <el-select v-model="form.site"  placeholder="请选择站点" disabled >
              <el-option
          ...
          <div class="item-group set-margin-bottom">
            <el-form-item label="产品分类1：" prop="product1" label-width="135px">
              <el-select v-model="form.product1"   placeholder="请选择" disabled>
                <el-option
            ...
            <el-form-item>
              <el-button type="primary" size="mini" @click="chooseCategory('1')" disabled>选择分类</el-button>
            </el-form-item>
            <el-form-item>
              <el-input v-model="form.categoryOne" maxlength="15" placeholder="请输入分类ID" style="width: 217px" oninput="value=value.replace(/[^\d]/g,'')" disabled></el-input>
              <el-button type="primary" size="mini" @click="confiromCategory('1')" disabled>确定</el-button>
            </el-form-item>
          </div>
          ...
          <div class="item-group set-margin-bottom">
            <el-form-item label="产品分类2：" clearable label-width="135px" >
              <el-select v-model="form.product2"  placeholder="请选择" disabled>
                <el-option
            ...
            <el-form-item>
              <el-button type="primary" size="mini" @click="chooseCategory('2')" disabled>选择分类</el-button>
            </el-form-item>
            <el-form-item>
                <el-input v-model="form.categoryTwo" maxlength="15" placeholder="请输入分类ID" style="width: 217px" oninput="value=value.replace(/[^\d]/g,'')" disabled></el-input>
                <el-button type="primary" size="mini" @click="confiromCategory('2')" disabled>确定</el-button>
            </el-form-item>
            <el-form-item>
              <span class="comment">
        ...
        <div>
          <div>
            <el-form-item label="售卖形式：" prop="salesMethod" label-width="135px">
              <el-radio-group v-model="form.salesMethod" disabled>
          ...
          <div>
            <el-form-item label="产品标题：" style="width:650px;margin-right: 60px" class="product_title_clearbg"  label-width="135px" prop="productTitle">
              <el-input
                  ...
                  >
                    ...
                    <span class="el-upload-list__item-actions">
                      <span class="el-upload-list__item-preview">
                        <i class="el-icon-zoom-in" @click="handlePictureCardPreview(src.img_path)"></i>
                      </span>
      ...
      <div>
        ...
        <div>
          ...
          <el-form-item>
              <el-button type="primary" size="mini" @click="generateUPC" disabled>生成UPC</el-button>
          </el-form-item>
          <el-form-item>
              <el-button type="warning" size="mini" @click=" upcCode = ''" disabled>默认</el-button>
          </el-form-item>
        </div>
        ...
        <div>
          ...
          <div>
            <!-- <el-form-item label="自定义属性："> -->
              <el-card class="card" shadow="always" style="width:600px;margin-left: 115px;" v-if="form.customizeAttr.length">
      ...
      <div>       
        ...
        <div>
          ...
          <div class="HTML-template" v-show="form.descriptionType === 'HTML'">
            <div class="button_display">
              <div></div>
              <el-button type="primary" @click="insertImage">一键插图</el-button>
            </div>
            <tinymce  v-model="form.richTextContent"  :height="450" ref="content" @input="input"/>
          </div>
        </div>
      </div>
      <div>    
      ...
      <div>
        ...
        <div>
          <el-form-item label="定时发布：" prop="is_plan_release">
            <el-checkbox v-model="is_plan_release" disabled>定时发布</el-checkbox>
          </el-form-item>
          <el-form-item
            label="发布时间："
            ...
            style="width:100%"
          >
            <el-date-picker disabled v-model="form.plan_update" type="datetime" placeholder="选择日期时间"></el-date-picker>
          </el-form-item>
        </div>
      </div>
...
import { setlistingEditSku,  getlistingEditSku} from '@/utils/auth'
...
export default {
  ...
  created() {
    ...
    let query = { online_listing_id: this.$route.query.online_listing_id }
  ...
  activated() {
    let id = this.$route.query.online_listing_id  
    if (JSON.parse(getlistingEditSku())) {
      if (id != JSON.parse(getlistingEditSku())) { //如果是点击标签页面进来得，带得是不同id，那么应该重新发起请求，并清空push进去的数据，避免重复         
                this.clearDataFunc()
                let query = { online_listing_id: id }
                this.getProductInfo(query)
            }      
    }
            
  },
  beforeRouteLeave(to, from, next) { // 路由离开的时候，将当前路由参数存入缓存，和下一次的进行比较
      // this.$store.commit('listing/clearUpcCode') // 清空upc编码，避免缓存
      if (this.$route.query.online_listing_id) setlistingEditSku(JSON.stringify(this.$route.query.online_listing_id))     
      next() 
  },
  mounted() {
  ...
  methods: {
    ...
    getAvailableLocation(id) {
      getAvailableLocation({ site_id: id }).then(res => {
    ...
    insertImage() {
      ...
      let imgList = this.form.images.map(item => item.img_path)
      getImagePath({ image_url: imgList }).then(res => {
        this.$refs.content.imageSuccessEditListing(res.data)
      })
      // let strContent = ''
      // if (this.form.richTextContent) {
      //   strContent = JSON.parse(JSON.stringify(this.form.richTextContent))
      //   this.$refs.content.setContent('') // 先清空
      // }
      // this.$refs.content.imageSuccessCBK(this.form.images) 
    },
    // 生成标题
    // generateTitle() {
      ...
      this.$refs[form].validate((valid) => {
          if (valid) {
            ...
            if (this.form.salesMethod === 1) {
              ...
              // data.gj_best_offer_auto_accept_price = this.form.gj_best_offer_auto_accept_price,
              data.gj_best_offer_auto_decline = this.form.gj_best_offer_auto_decline ? 1 : 0,
              // data.gj_minP_best_offer_price = this.form.gj_minP_best_offer_price,
            ...
            if (this.form.product2) {
              ...
              this.$store.dispatch('listing/upadteListingDetail', data)
          } else {
             this.$message({
              type: 'warning',
              message: '请填写必填的字段'
    ...
    async getProductInfo(query) {
        getCopyListingDetail(query).then(res => {
          let data = res.data
          let list = []
            ...
            this.$store.dispatch('listing/getListingConfig', { site_id: data.site_id })   
            this.form.published_sku = data.sku
            ...
            this.form.private_listing = data.private_listing ? true : false
            this.form.listing_duration = data.listing_duration
            this.flagListingListingduration = data.listing_duration > 0 ? data.listing_duration : 1
            ...
            //   let query = {
            //     site_id: this.siteData.filter(item => item.site === data.site)[0].site_id
            //   }
            //   this.$store.dispatch('listing/getListingConfig', query)
            // }
            ...
            getAvailableLocation({ site_id: data.site_id }).then(res => {
              ...
              data.international_shipping.forEach(item => {
                ...
                if (typeof(item.shipping_location) === 'string') { // 如果当前的类型为字符串，那么选择的就是全球
                  item.global = true
                  item.shipping_location =  _g.getJsonParse(res.data)
                } else { // 不是则是数组类型，那么选的就是国家
                  item.checkAll = true
                  let availableLocation = _g.getJsonParse(res.data)
                  availableLocation.forEach(ele => {
                    item.shipping_location.forEach(location => {
                      let list = []
                      list.push(ele.value)
            ...
            if (data.category_ids.length) {
              ...
              this.product1 = [{ id: data.category_history_id, label: data.category_path[data.category_path.length - 1]}]
            }       
            if (data.second_category_ids.length) {
              ...
              this.product2 = [{ id: data.second_category_history_id, label: data.second_category_path[data.second_category_path.length - 1]}]
            } else {
              this.product2 = []
              this.form.product2 = ''
              this.saveProduct2IdList = {}
            ...
            if (data.shop_category_ids.length) {
              ...
              this.shop1 = [{ id: data.shop_category_history_id, label: data.shop_category_path[data.shop_category_path.length - 1]}]
            } else {
              this.form.shop1 = ''
              this.saveShop1IdList = {}
              this.saveShop1Name = {}
            ...
            if (data.shop_second_category_ids.length) {
              ...
              this.shop2 = [{ id: data.shop_second_category_history_id, label: data.shop_second_category_path[data.shop_second_category_path.length - 1]}]
            } else {
              this.form.shop2 = ''
              this.saveShop2IdList = {}
              this.saveShop2Name = {}
          ...
          if (this.$refs.content) {
            this.$refs.content.setContent(data.description)
          }    
        // 描述信息   
