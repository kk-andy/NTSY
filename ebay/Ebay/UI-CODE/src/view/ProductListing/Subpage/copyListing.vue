...
import { setCopyListingSku,  getCopyListingSku} from '@/utils/auth'
...
export default {
  ...
  watch: {
        ...
        'form.listing_duration': {
          handler(val) {
            if (val > 0) {
              this.flagListingListingduration = val
            }
          }
        },
        'allowMultiattr': {
  ...
  activated() {
    ...
    if (JSON.parse(getCopyListingSku())) {
      if (id != JSON.parse(getCopyListingSku())) { //如果是点击标签页面进来得，带得是不同id，那么应该重新发起请求，并清空push进去的数据，避免重复         
                this.clearDataFunc()
  ...
  beforeRouteLeave(to, from, next) { // 路由离开的时候，将当前路由参数存入缓存，和下一次的进行比较
      ...
      if (this.$route.query.online_listing_id) setCopyListingSku(JSON.stringify(this.$route.query.online_listing_id))     
      next() 
  },
  ...
  methods: {
      ...
      this.$refs[form].validate((valid) => {
          if (valid) {
            ...
            let data = {
              ...
              base_imgs: this.form.images.map(item => item.img_path),
              upc: this.upcCode,
            ...
            if (num === 1 || num === 3) {
              this.$store.dispatch('listing/copyCreateListing', data)
            } else {
    ...
    handleTableDelete(index, data) {
      this.productAndSku = this.productAndSku.filter(item => item.product_sku !== data.sku)
