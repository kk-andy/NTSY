...
import { setPublicationRecordsEditSku, getPublicationRecordsEditSku } from '@/utils/auth'
...
export default {
  ...
  activated() {
    ...
    if (JSON.parse(getPublicationRecordsEditSku())) {
      if (id != JSON.parse(getPublicationRecordsEditSku())) { //如果是点击标签页面进来得，带得是不同id，那么应该重新发起请求，并清空push进去的数据，避免重复 
                this.clearDataFunc()         
  ...
  beforeRouteLeave(to, from, next) { // 路由离开的时候，将当前路由参数存入缓存，和下一次的进行比较
      // this.$store.commit('listing/clearUpcCode')
      if (this.$route.query.sku) setPublicationRecordsEditSku(JSON.stringify(this.$route.query.sku))     
      next() 
  },
  mounted() {   
  },
