      ...
      <div>
        ...
        <div>
          ...
          <el-form-item label="图片："  style="width:100%" class="img-form-item">
            <div class="drag download-image" :class="{'aligncenter': form.images.length}">
      ...
      <div>       
        ...
        <div>
          ...
          <div class="HTML-template" v-if="showHTMLtemplate">
            ...
            <tinymce v-model="form.richTextContent"  :height="450" ref="content" :id="content" @input="input"/>
          </div>
        </div>
      </div>
      <div>    
      ...
      <div>
        ...
        <div class="transportation-policy">
          ...
          <div class="foreign-transportation transport">
            ...
            <p style="margin-left: 115px;color: #933;word-break: break-all" v-if="form.ddl_exclusion_list_type === 2 ">{{form.unavailable_shipping_location}}</p>
          </div>
        </div>
      </div>
      <div>
...
import { setEditCreateListingSku, getEditCreateListingSku } from '@/utils/auth'
...
export default {
  ...
  activated() {
    let id = this.$route.query.spu
    if (JSON.parse(getEditCreateListingSku())) {
      if (id != JSON.parse(getEditCreateListingSku())) { // 如果是点击标签页面进来得，带得是不同id，那么应该重新发起请求，并清空push进去的数据，避免重复                 
                this.clearDataFunc()
                let query = { spu: id }
  ...
  beforeRouteLeave(to, from, next) { // 路由离开的时候，将当前路由参数存入缓存，和下一次的进行比较
      ...
      if (this.$route.query.spu) setEditCreateListingSku(JSON.stringify(this.$route.query.spu))     
      next() 
  },
  mounted() {
    // this.$store.dispatch('listing/getSkuInfo', { sku: this.$route.query.spu })
    let query = { spu: this.$route.query.spu }
    this.form.published_sku= this.$route.query.sku
    this.getProductInfo(query)
    setTimeout(() => {
      this.getCurrency()
      this.$nextTick(() => {     
  ...
  methods: {
    ...
    insertImage() {
      ...
      // this.$refs.content.imageSuccessEditListing(this.form.images) // 不清空会导致会有重复图片，清空得话，又会将之前的文本类容清空掉
      // let imgHTMLArr = []
      // this.form.images.forEach(img => {
      //   imgHTMLArr.push(`<img style="width:50%" src="${img}" alt>`);
      // });
    ...
    getProductInfo(query) {
      getSpuInfo(query).then(res => {
          ...
          for (let key in data.variety_attrs) {
            this.testMutiData[key] = data.variety_attrs[key]
            let obj = {}
            obj.name = key
            obj.valueList = []
          ...
          this.$set(this.form, 'richTextContent', data.description)
