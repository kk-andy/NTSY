...
export default {
        ...
        saveFormFunc(formName) {
            this.$refs[formName].validate((valid) => {
                if (valid) {
                    ...
                    this.$store.dispatch('listOfSelections/saveSkuInfo', { data, flag: false })
                } else {
            ...
            await getListingInfo(query).then(res => {
                try {
                    ...
                    if (this.shopList.length) {
                        if (this.shopList.find(item => item.shop_id === data.shop_id).site_code === 'MY') {
                            this.showCites = true
                        }
                    }
    ...
    created() {
        let query = { 
            online_listing_id: this.$route.query.listing_id,
            is_copy: 1
            }
